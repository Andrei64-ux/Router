 ...
 * 
 * @param buffer 
 * @return struct iphdr* A pointer to a structure of type iphdr that is inside the buffer. Basically, we return the
 * ip header from buffer.
 * If this is not an IP packet, we return NULL.
 */
struct iphdr * parse_ip(void *buffer);
